...
See [Page Layout](/wiki/LaTeX/Page_Layout).
...
Avoid using latin1. See [Special Characters](/wiki/LaTeX/Special_Characters).
...
There are ways to get around the default behavior, however; see [\\paragraph line break](/wiki/LaTeX/Paragraph_Formatting#.5Cparagraph_line_break) for more information.
...
See [Fonts](/wiki/LaTeX/Fonts#Font_encoding) for an explanation.
...
See [Importing Graphics](/wiki/LaTeX/Importing_Graphics) and [Floats, Figures and Captions](/wiki/LaTeX/Floats,_Figures_and_Captions) for more details.

## Text stops justifying

Most likely you have used `\raggedleft`, `\raggedright` or `\centering` at some point and forgotten to switch it off. These commands are switches — they remain active until the end of the scope, or until the end of the document if there is no scope. See [Paragraph Alignment](/wiki/LaTeX/Paragraph_Formatting#Paragraph_Alignment) for more information.
...
Trying to stretch tables with the default tabular environment will often lead to unexpected results. The nice tabu package will do what you want and even much more. Alternatively if you cannot use the tabu package you may try tabularx or tabulary packages. See [Tables](/wiki/LaTeX/Tables).
...
Some LaTeX editors feature table assistants. Also, many spreadsheet applications have a LaTeX export feature (or plugin). Again, see [Tables](/wiki/LaTeX/Tables) for more details.

## Relieving cumbersome code (lists and long command names)

LaTeX is sometimes cumbersome to write, especially if you are not using an adequate editor. See [Editors](/wiki/LaTeX/Installation#Editors) for some interesting choices.
...
The [Installation](/wiki/LaTeX/Installation) article explains in detail how to manually install a fully functional TeX environment, including LaTeX and other features, in under 100 MB.
