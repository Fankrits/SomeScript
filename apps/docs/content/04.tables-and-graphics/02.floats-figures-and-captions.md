...
use `\renewcommand{\figurename}{Example}` in the figure contents. See the [New commands](/programming/macros#new-commands) section for more information on this.
...
Floating tables are covered in a [separate chapter](/tables-and-graphics/tables#floating-with-table).
...
The `placeins`[link](http://www.ctan.org/pkg/placeins) package provides the command `\FloatBarrier`, which can be used to prevent floats from being moved over it. This can, e.g., be useful at the beginning of each section. The package even provides an option to change the definition of `\section` to automatically include a `\FloatBarrier`. This can be set by loading the package with the option `[section]` (`\usepackage[section]{placeins}`).  `\FloatBarrier` may also be useful to prevent floats intruding on lists created using `itemize` or `enumerate`.

The `flafter` package can be used to force floats to appear after they are defined, and the `endfloat`[link](http://www.ctan.org/pkg/endfloat) package can be used to place all floats at the end of a document.

The `float`[link](http://www.ctan.org/pkg/float) package provides the `H` option to floating environments, which completely stops them from floating.

The [`caption`](http://www.ctan.org/pkg/caption) package provides the command `\caption{<type>}{<caption text>}` that lets you typeset a caption without a floating environment. You have the full and absolute control about the placement of your figures and captions.
...
Labels and cross-references work fairly similarly to the general case - see the [Labels and Cross-referencing](/cross-references/labels-and-cross-referencing) section for more information.
...
Anyway, we will look at the package `wrapfig`. Note that `wrapfig` may not come with the default installation of LaTeX; you might need to [install additional packages](/getting-started/installing-extra-packages). Noted also, wrapfig is incompatible with the enumerate and itemize environments
...
*[Math spaces](/technical-writing/mathematics): `\qquad`, `\quad`, `\;`, and `\,`
...
2. Declare your new float using: `\newfloat{type}{placement}{ext}[outer counter]`, where:
   ...
   - *placement* - t, b, p, or h (as previously described in [Placement](#floats)), where letters enumerate permitted placements.
...
The `verbatim` is an environment already discussed in [Special paragraphs](/document-fundamentals/paragraph-formatting#verbatim-text). It is good for source code, but if you want to introduce a lot of code you might consider using the `listings` package described in [Source Code Listings](/technical-writing/source-code-listings), that was made just for it.
...
See [this page](/reference/counters) for more information on counters.
...
A much more complicated package which can be used in the same way is [TikZ](/tables-and-graphics/pgf-tikz). TikZ is a front-end to a drawing library called pgf (which is used to make beamer for instance). It can be used to label figures by adding text nodes on top of an image node.
