...
If you want to be able to see the markers you are using in the output document as well, you can use the `showkeys` package; this can become very useful as you develop your document. For more information see the [Packages](/packages) section.
...
[150px](/image:latex-example-referencing-section.png)

You could place the label anywhere in the section; however, in order to avoid confusion, it is better to place it immediately after the ''beginning'' of the section. Note how the marker starts with ''sec:'', as suggested before. The label is then referenced in a different section, where the tilde (~) indicates a [non-breaking space](https://en.wikipedia.org/wiki/non-breaking space).
...
For more, see the [Floats, Figures and Captions](/floats,-figures-and-captions) section about the `figure` and related environments.
...
The `amsmath` package adds a new command for referencing formulae; it is `\eqref{}`. It works exactly like `\ref{}`, but adds parentheses so that instead of printing a plain number as ''5'', it will print ''(5)''. This can be useful to help the reader distinguish between formulae and other things, without the need to repeat the word "formula" before any reference.<ref>[https://tex.stackexchange.com/questions/107422/what-is-the-difference-between-eqref-and-ref TeX StackExchange — What is the difference between \eqref and \ref?]</ref> Its output can be changed as desired; for more information see the `amsmath` documentation.
...
The `[hyperref](/packages/hyperref)` package introduces another useful command; `\autoref{}`. This command creates a reference with additional text corresponding to the target's type, all of which will be a hyperlink.  For example, the command `\autoref{sec:intro}` would create a hyperlink to the `\label{sec:intro}` command, wherever it is. Assuming that this label is pointing to a section, the hyperlink would contain the text "section 3.4", or similar (the full list of default names can be found [https://mirrors.ctan.org/macros/latex/contrib/hyperref/doc/hyperref-doc.html#TBL-28 here]). Note that, while there's an ```latex" enclose="none\autoref*``` command that produces an unlinked prefix (useful if the label is on the same page as the reference), no alternative ```latex" enclose="none\Autoref``` command is defined to produce capitalized versions (useful, for instance, when starting sentences); but since the capitalization of autoref names was chosen by the package author, you can customize the prefixed text by redefining `\''type''autorefname` to the prefix you want, as in:
```latex
...
Because `varioref`,`hyperref`, and `cleveref` redefine the same commands, they can produce unexpected results when their `\usepackage` commands appear in the preamble in the wrong order.  For example, using `hyperref`,`varioref`, then `cleveref` can cause `\vref{}` to fail as though the marker were undefined.<ref name=":0">Tests done under report class http://tex.stackexchange.com/questions/139459/vref-and-input-command</ref>  The following order generally seems to work:
# `varioref`
# `hyperref`
# `cleveref`<ref name=":0" />
