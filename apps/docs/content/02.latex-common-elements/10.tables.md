...
\end{tabular}
|render=
![[250px](/image:latex-example-tabular-cline.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[250px](/image:latex-example-tabular-cline.svg))
} }
...
![center](/image:latex-example-wrapped-table.svg)](https://en.wikibooks.org/wiki/Special:FilePath/center](/image:latex-example-wrapped-table.svg))
...
One can also introduce a horizontal space with hspace like this: `\begin{tabular}{r@{\hspace{1in}<!---->}<!---->rr}<!---->` There are different options for the hspace [length](/lengths).
...
{|
| style="vertical-align: middle;" | `<nowiki>{|l|l|}</nowiki>`
| style="vertical-align: middle;" |
![[Image:specifier1.svg](/image:specifier1.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:specifier1.svg](/image:specifier1.svg))
|-
| style="vertical-align: middle;" | `<nowiki>{|@{}l|l@{}|}</nowiki>`
| style="vertical-align: middle;" |
![[Image:specifier2.svg](/image:specifier2.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:specifier2.svg](/image:specifier2.svg))
|-
| style="vertical-align: middle;" | `<nowiki>{|@{}l@{}|l@{}|}</nowiki>`
| style="vertical-align: middle;" |
![[Image:specifier3.svg](/image:specifier3.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:specifier3.svg](/image:specifier3.svg))
|-
| style="vertical-align: middle;" | `<nowiki>{|@{}l@{}|@{}l@{}|}</nowiki>`
| style="vertical-align: middle;" |
![[Image:specifier4.svg](/image:specifier4.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:specifier4.svg](/image:specifier4.svg))
|}

### Aligning columns at decimal points using dcolumn
Instead of using @-expressions to build columns of decimals aligned to the decimal point (or equivalent symbol), it is possible to center a column on the decimal separator using the `dcolumn` package, which provides a new column specifier for floating point data. See the [http://anorien.csc.warwick.ac.uk/mirrors/CTAN/macros/latex/required/tools/dcolumn.pdf dcolumn package documentation] for more information, but a simple way to use `dcolumn` is as follows.
...
\end{tabular}
|render=
![[Image:multicolumn.svg](/image:multicolumn.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:multicolumn.svg](/image:multicolumn.svg))
} }

### Columns spanning multiple rows

The first thing you need to do is add `\usepackage{multirow`} to the preamble<ref name="multirow">Package multirow [http://www.ctan.org/tex-archive/macros/latex/contrib/multirow/ on CTAN]</ref>. This then provides the command needed for spanning rows: `\multirow{''num_rows''}{''width''}{''contents''}`. The arguments are pretty simple to deduce (`*<!---->` for the ''width'' means the content's natural width).
...
\end{tabular}
|render=
![[Image:multirow.svg](/image:multirow.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:multirow.svg](/image:multirow.svg))
} }
...
If there is no data for a cell, just don't type anything, but you still need the "&" separating it from the next column's data. The astute reader will already have deduced that for a table of <math>n</math> columns, there must always be <math>n-1</math> ampersands in each row (unless `\multicolumn<!---->` is also used).
...
\end{tabular}
|render=
![[300px](/image:multirowandcolumnexample.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[300px](/image:multirowandcolumnexample.svg))
} }
...
\end{tabular}
|render=
![[400px](/image:latex-tables-double-dichotomy-example.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[400px](/image:latex-tables-double-dichotomy-example.svg))
} }
...
See [Fonts](/fonts#sizing_text) for named font sizes.  The table caption font size is not affected. To control the caption font size, see [Caption Styles](/floats,_figures_and_captions#caption_styles).
...
\end{tabular*}
|render=
![[right](/image:latex-tabwidth1.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[right](/image:latex-tabwidth1.svg))
} }
...
\end{tabular*}
|render=
![[right](/image:latex-tabwidth2.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[right](/image:latex-tabwidth2.svg))
} }
...
\end{tabularx}
|render=
![right](/image:latex-tabxwidth1.svg)](https://en.wikibooks.org/wiki/Special:FilePath/right](/image:latex-tabxwidth1.svg))
} }
...
\end{tabularx}
|render=
![right](/image:latex-tabxwidth2.svg)](https://en.wikibooks.org/wiki/Special:FilePath/right](/image:latex-tabxwidth2.svg))
} }
...
```
>{\setlength\hsize{1\hsize}\centering}X{ {!} }>{\setlength\hsize{1\hsize}\raggedleft}X@{} >{\setlength\hsize{1\hsize}\raggedright}X{ {!} }>{\setlength\hsize{1\hsize}\centering}X{ {!} } } 
  \hline
...
\end{tabularx}
|render=
![right](/image:latex-tabularx_multi.svg)](https://en.wikibooks.org/wiki/Special:FilePath/right](/image:latex-tabularx_multi.svg))
} }
...
[http://www.ctan.org/pkg/tabulary `tabulary`] is a modified `tabular*` allowing width of columns set for equal heights.
...
The `tabu` package (which is [https://github.com/tabu-issues-for-future-maintainer/tabu/blob/main/README.md no longer maintained]) provides the `longtabu` environment. It has most of the features of `tabu`, with the additional capability to span multiple pages.
...
Alternatively you can try one of the following packages [http://www.ctan.org/pkg/supertabular `supertabular`] or [http://www.ctan.org/pkg/xtab `xtab`], an extended and somewhat improved version of `supertabular`.
...
\end{tabular}
|render=
![[Image:Partial-vertical-line-add.svg](/image:partial-vertical-line-add.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:Partial-vertical-line-add.svg](/image:partial-vertical-line-add.svg))
} }
...
\end{tabular}
|render=
![[Image:Partial-vertical-line-remove.svg](/image:partial-vertical-line-remove.svg)](https://en.wikibooks.org/wiki/Special:FilePath/[Image:Partial-vertical-line-remove.svg](/image:partial-vertical-line-remove.svg))
} }
...
Tables can also be put on their side within a document using the `rotating` or the `rotfloat` package. See the [Rotations](/rotations) chapter.

## Table with legend

To add a legend to a table the [http://www.ctan.org/tex-archive/macros/latex/contrib/caption/ caption] package can be used. With the caption package a `\caption*{...}<!---->` statement can be added besides the 
...
The `table` environment initiates a type of float just as the environment `figure`. In fact, the two bear a lot of similarities (positioning, captions, etc.). More information about floating environments, captions etc. can be found in [Floats, Figures and Captions](/floats,_figures_and_captions).
...
See [Modular Documents](/modular-documents) for more details.

## Need more features?

Have a look at one of the following packages:

* [http://www.ctan.org/pkg/hhline `hhline`]:   do whatever you want with horizontal lines
* [http://www.ctan.org/pkg/array `array`]:    gives you more freedom on how to define columns
* [http://www.ctan.org/pkg/colortbl `colortbl`]: make your table more colorful
* `[https://ctan.org/pkg/threeparttable threeparttable]` makes it possible to put footnotes both within the table and its caption
* [http://www.ctan.org/pkg/arydshln `arydshln`]: creates dashed horizontal and vertical lines
* [http://www.ctan.org/pkg/ctable `ctable`]: allows for footnotes under table and properly spaced caption above (incorporates booktabs package)
* [http://www.ctan.org/pkg/slashbox `slashbox`]: create 2D tables with the first cell containing a description for both axes. Not available in Tex Live 2011 or later. 
* `[https://ctan.org/pkg/diagbox diagbox]`: compatible to `slashbox`, come with Tex Live 2011 or later
* [http://www.ctan.org/pkg/dcolumn `dcolumn`]: decimal point alignment of numeric cells
* [http://www.ctan.org/pkg/rccol `rccol`]: advanced decimal point alignment of numeric cells with rounding
* [http://www.ctan.org/pkg/numprint `numprint`]: print numbers, in the current mode (text or math) in order to use the correct font, with separators, exponent and/or rounded to a given number of digits. tabular(*), array, tabularx, and longtable environments are supported using all features of numprint
* [http://www.ctan.org/pkg/spreadtab `spreadtab`]: spread sheets allowing the use of formulae
* [http://ctan.org/tex-archive/macros/latex/contrib/siunitx `siunitx`]: alignment of tabular entries
* `[https://www.ctan.org/pkg/pgfplotstable pgfplotstable]`: loads, rounds, formats and postprocesses numerical tables, e.g. by importing the data directly from .csv (comma-separated value) files instead of manually writing the whole tables in LaTeX code. Programs such as Excel, LibreOffice Calc etc. can export data sheets as .csv files.
...
[fr:LaTeX/Faire_des_tableaux](/fr:/faire_des_tableaux)
[nl:LaTeX/Tabellen](/nl:/tabellen)
[pl:LaTeX/Tabele](/pl:/tabele)
