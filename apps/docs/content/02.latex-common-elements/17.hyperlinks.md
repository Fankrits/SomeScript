...
It just shows your email address (so people can know it even if the document is printed on paper) but, if the reader clicks on it, (s)he can easily send you an email. Or, to incorporate the `url` package's formatting and line breaking abilities into the displayed text, use<ref>[Email link with hyperref, url packages](http://groups.google.com/group/comp.text.tex/browse_thread/thread/ae160fd2fc5680a5/71a5a7c7bfceb3cb?lnk=gst&q=email+url+hyperref#71a5a7c7bfceb3cb)</ref>
...
|-
|`pdfstartview `
|`={FitH},{''FitV''},etc<ref>Other possible values are defined in the [http://mirror.switch.ch/ftp/mirror/tex/macros/latex/contrib/hyperref/doc/manual.html#TBL-7-40-1 hyperref manual]</ref>.`
...
If you use the `wrapfig` package<ref>[http://www.ctan.org/tex-archive/macros/latex/contrib/wrapfig Wrapfig package webpage in CTAN]</ref> mentioned in the "[Wrapping text around figures](/floats,-figures-and-captions#wrapping-text-around-figures)" section of the "Floats, Figures and Captions" chapter, or other similar packages that define their own environments, you will need to manually include `\capstart<!---->` in those environments, ''e.g.'':
