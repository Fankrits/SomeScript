...
The LaTeX kernel provides a powerful hook management system (introduced in 2020). Hooks let you execute code at specific points in the document processing:

### Document Hooks

```latex
\AddToHook{begindocument}{Code to run at begin document}
\AddToHook{enddocument}{Code to run at end document}
\AddToHook{begindocument/end}{Code after all packages load}
```

### Environment Hooks

```latex
\AddToHook{env/tabular/begin}{Code before every tabular}
\AddToHook{env/tabular/end}{Code after every tabular}
```

### File Hooks

```latex
\AddToHook{file/before}{Code before file is input}
\AddToHook{file/after}{Code after file is input}
```

### Package/Class Hooks

```latex
\AtEndOfPackage{Code to run after package loads}
\AtEndOfClass{Code to run after class loads}
```

## Key-Value Options (Modern)

Since June 2022, the LaTeX kernel supports key-value options via `\DeclareKeys`:

```latex
\DeclareKeys{
  mypackage.color .initial = blue,
  mypackage.color .default = red,
  mypackage.verbose .Boolean = false,
  mypackage.size .store_in = \mypkg@size,
  mypackage.size .initial = 10pt,
}
...
| Type | Description |
|------|-------------|
| `.initial` | Sets initial value |
| `.default` | Value when key is used without `=` |
| `.store_in` | Stores value in a command |
| `.Boolean` | True/false key |
| `.choice` | Multiple choice key |

### Example with Choices

```latex
\DeclareKeys{
  mypkg.alignment .choices = {
    left, center, right
  },
  mypkg.alignment .initial = center,
  mypkg.alignment .code = \def\mypkg@align{#1},
}

\ProcessKeyOptions[mypackage]
```

This is the recommended approach for modern package development, replacing the older `\DeclareOption` pattern for complex options.
