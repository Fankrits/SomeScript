...
The `showidx` package that comes with LaTeX prints out all index entries in the margin next to the text (the outer margin in two-sided documents). This is quite useful for proofreading a document and verifying the index.
...
Unfortunately, the current version of `xindy` and `hyperref` are incompatible. When you use `textbf` or `textit` modifiers, `texindy` will print the error message `unknown cross-reference-class 'hyperindexformat'! (ignored)` and won't add those pages to the index.
