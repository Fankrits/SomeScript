...
The remaining fields contain the entry's data. Field names are BibTeX keywords followed by `=`, and values are normally enclosed in quotation marks or braces. Prefer braces for values containing TeX commands or capitalization that BibTeX must preserve; for example, `title = {G{\"o}del's theorem}`. With a modern UTF-8 workflow, Unicode text may also be stored directly when the selected bibliography processor and style support it. The document preamble does not change how the `.bib` parser reads its database.
