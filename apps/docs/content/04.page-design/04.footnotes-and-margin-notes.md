...
description: "Learn to add, format, and troubleshoot footnotes, margin notes, and endnotes in LaTeX documents."
---
...
Nevertheless, in that case, the current footnote counter is still incremented, so for instance you'd get footnote 1, unnumbered, and 3.  A better solution consists in defining the following macro in the preamble, and to use it:
...
The package [footmisc](http://www.ctan.org/pkg/footmisc) offers many possibilities for customizing the appearance of footnotes. It can be used, for example, to use a different font within footnotes.
...
* **Footnotes in `tabular` cells need special handling.** Use `\footnotemark` in the table and place the matching `\footnotetext{...}` immediately afterwards. To make a table environment collect its footnotes automatically, load `footnotehyper` and add `\makesavenoteenv{tabular}` in the preamble. `footnotehyper` is the modern, `hyperref`- and `xcolor`-compatible successor to `footnote`; it also provides the `savenotes` environment. See the [package documentation](https://ctan.org/pkg/footnotehyper) for its compatibility notes and alternatives such as `threeparttable`.

* **Footnotes in a `minipage` are supported, but separate from the document's footnotes.** They are placed at the bottom of the minipage and use their own numbering sequence. When you need footnotes from another environment to appear at the page foot, `footnotehyper` can adapt that environment with `\makesavenoteenv{environmentname}`.
