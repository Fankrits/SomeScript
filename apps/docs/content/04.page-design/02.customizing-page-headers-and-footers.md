...
To get better control over the headers, one can use the package `fancyhdr` written by
Piet van Oostrum. It provides several commands that allow you to customize the header and footer lines of
your document. For a more complete guide, the author of the package produced this [documentation](http://www.ctan.org/tex-archive/macros/latex/contrib/fancyhdr/fancyhdr.pdf).
...
The styles can be customized with `fancyhdr` specific commands.
Those two styles may be configured directly, whereas for LaTeX styles you need to make a call to the `\fancypagestyle` command.
...
Package `scrlayer-scrpage` is part of the KOMA-script
...
the same content to be on `plain` pages as well.
