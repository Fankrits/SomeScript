...
Before starting, ensure you have LaTeX installed on your computer (see [Installation](/wiki/LaTeX/Installation) for instructions of what you will need).
...
If you want to insert text that might contain several particular symbols (such as URIs), you can consider using the `\verb` command, which will be discussed later in the section on formatting. For source code, see [Source Code Listings](/wiki/LaTeX/Source_Code_Listings).

The less-than `<` and greater-than `>` characters are the only visible ASCII characters (not reserved) that will not print correctly. See [Special Characters](/wiki/LaTeX/Special_Characters) for an explanation and a workaround.

Non-ASCII characters (e.g. accents, diacritics) can be typed in directly for most cases. However you must configure the document appropriately. The other symbols and many more can be printed with special commands as in mathematical formulae or as accents. We will tackle this issue in [Special Characters](/wiki/LaTeX/Special_Characters).
...
The core LaTeX language does not have a predefined syntax for commenting out regions spanning multiple lines. Refer to [multiline comments](/wiki/LaTeX/Paragraph_Formatting) for simple workarounds.
...
See [Document Structure](/wiki/LaTeX/Document_Structure) and the **Common Elements** part for all the common features that belong to every type of document.

### Non-English documents and special characters

LaTeX has some nice features for most languages in the world. You can tell LaTeX to follow typography rules of the target language, ease special characters input, and so on. See [Special Characters](/wiki/LaTeX/Special_Characters) and [Internationalization](/wiki/LaTeX/Internationalization).

### Modular document

See [Modular Documents](/wiki/LaTeX/Modular_Documents) for good recommendations about the way to organize big projects into multiple files.

### Questions and Issues

We highly urge you to read the [FAQ](/wiki/LaTeX/FAQ) if you have issues about basic features, or if you want to read essential recommendations. For the more specific questions and issues, refer to the [Tips and Tricks](/wiki/LaTeX/Tips_and_Tricks) page.

### Macros for the utmost efficiency

The full power of LaTeX resides in macros. They make your documents very dynamic and flexible. See the [dedicated part](/wiki/LaTeX/Macros).

### Working in a team

See chapter [Collaborative Writing of LaTeX Documents](/wiki/LaTeX/Collaborative_Writing_of_LaTeX_Documents).
