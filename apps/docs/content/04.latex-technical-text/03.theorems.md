...
You can also create a theorem environment that is not numbered by using the `newtheorem*` command<ref name="amsthm">Requires the `amsthm` package</ref>.  For instance,
```latex
\newtheorem*{mydef}{Definition}
```
...
## Proofs
The `proof` environment<ref name="amsthm"/> can be used for adding the proof of a theorem. The basic usage is:
```latex
\begin{proof}
Here is my proof
\end{proof}
...
### Custom styles
To define your own style, use the `\newtheoremstyle` command<ref name="amsthm"/>:
```latex
