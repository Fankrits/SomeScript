---
title: "PSTricks"
description: "Learn to create PostScript-based graphics with PSTricks and choose a compatible modern LaTeX build workflow."
---
...
1. **`auto-pst-pdf`** — Generate and include PDF versions of PSTricks graphics when compiling with pdfLaTeX. It invokes external tools, so compile with `--shell-escape`:
   ```latex
...
2. **XeLaTeX** — Current TeX distributions include the `xetex-pstricks` driver configuration, allowing PSTricks to work automatically with XeLaTeX:
   ```bash
   xelatex filename.tex
   ```

3. **LuaLaTeX with `auto-pst-pdf-lua`** — LuaLaTeX does not process PostScript drawings by itself. Load the Lua-specific conversion wrapper and enable shell escape:
   ```latex
...
For new pdfLaTeX documents, prefer `auto-pst-pdf`, described above. The older `pdftricks` package provides another conversion workflow, but it requires more source markup.
...
The lower-level `pst-pdf` package is another option. It creates a PDF container for PostScript graphics in an additional processing step; `auto-pst-pdf` automates that workflow.
