...
See [Counters](/reference/counters).
...
- **backmatter**: chapters will not be numbered, but unlike `frontmatter`, page numbering is left as-is (it keeps counting up in arabic from wherever `mainmatter` left it).
