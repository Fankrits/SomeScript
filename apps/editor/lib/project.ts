...
const CONFIG_PATH = path.join(process.cwd(), "active-project.json");

export async function getProjectPath(): Promise<string> {
  try {
    const data = await fs.readFile(CONFIG_PATH, "utf-8");
    const parsed = JSON.parse(data);
    return parsed.projectPath || path.join(process.cwd(), "my-new-project");
  } catch {
    // Default project folder
    const defaultPath = path.join(process.cwd(), "my-new-project");
