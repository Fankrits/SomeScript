...
test("saves a thread and reads it back", () => {
  ...
  expect(saveThreadHistory("t1", [event(1)], { sessionId: "s" }, s)).toBe(true);
...
test("reclaims space from threads the sidebar can no longer reach", () => {
  ...
  s.setItem("eve-threads-list", JSON.stringify([{ id: "live" }]));
  // An orphan left behind by an older session, hogging most of the budget.
  s.setItem(threadKey("orphan"), "y".repeat(2_000));

  expect(saveThreadHistory("live", [event(1), event(2)], null, s)).toBe(true);
...
test("never deletes a thread that is still listed", () => {
  ...
  s.setItem("eve-threads-list", JSON.stringify([{ id: "keep" }, { id: "live" }]));
...
test("trims oldest events when the newest still will not fit", () => {
  ...
  expect(saveThreadHistory("t1", events, null, s)).toBe(true);
...
test("reports failure when even one event cannot fit", () => {
  ...
  expect(saveThreadHistory("t1", [event(1)], null, s)).toBe(false);
});
...
test("a full store does not wedge later writes", () => {
  ...
  expect(saveThreadHistory("t1", big, null, s)).toBe(true);
  expect(saveThreadHistory("t1", [...big, event(60)], null, s)).toBe(true);
