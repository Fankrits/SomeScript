...
export const VoiceSelector = ({
  ...
  open: openProp,
  defaultOpen = false,
  // oxlint-disable-next-line typescript/unbound-method -- plain callback prop forwarded to useControllableState, not a class method.
  onOpenChange,
  children,
  ...props
}: VoiceSelectorProps) => {
...
export type VoiceSelectorAccentProps = ComponentProps<"span"> & {
  value?:
    ...
    | "turkish"
    | "greek"
    // `string & {}` (not bare `string`) keeps the literals above from collapsing
    // into plain `string`, so editor autocomplete still suggests them.
    | (string & {});
};
