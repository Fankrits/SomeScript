...
export const CommitTimestamp = ({ date, className, children, ...props }: CommitTimestampProps) => {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- relative time is a client-only value (avoids SSR hydration mismatch)
    updateFormatted();
  }, [updateFormatted]);
...
export const CommitActions = ({ className, children, ...props }: CommitActionsProps) => (
  // Only stops click/keydown from bubbling to the row's own expand handler —
  // the actual action buttons inside stay independently keyboard-accessible.
